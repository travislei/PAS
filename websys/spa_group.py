...
class Matching(object):
    ...
    def assign(self, lid, pid, sid):
        ...
        if sid not in self.g_matched[pid]:
            self.g_matched[pid].append(sid)
    ...
    def delete(self, lid, pid, sid):
        ...
        if sid in self.g_matched[pid]:
            self.g_matched[pid].remove(sid)
    ...
    def has_students(self, slist):
        for s in slist:
            if s in self.s_matched:
                return True

        return False

    def get_students(self, pid):
        return self.g_matched[pid]
    ...
    def print_group(self):
        logging.debug('Current group\' matching:')

        for s, v in self.g_matched.items():
...
if len(sys.argv) == 2:
    with open(sys.argv[1], 'r') as f:
        ...
        for s in json_string['group_list']:
            ...
            group.members = idstr_to_list(s['members'])
...
while lecturer_list:
    ...
    for pid, preflist in l.preflist:
        ...
        if preflist and p.is_undersubscribed():
            ...
            for gid in preflist:
                if (gid is None) or (not l.is_undersubscribed()):
                    break

                g = group_dict[gid]
                logging.debug('\t{}'.format(group_dict[g.id]))

                #  Check if the lecturer has enough capacity
                if g.capacity > (l.capacity - l.matched):
                    logging.debug('\tDo not have enought capacity!')
                    continue

                if g.pid is None:
                    #  Check if any group member has been matched
                    if matching.has_students(g.members):
                        logging.debug('\tNot all group members available!')
                        continue

                    matching.assign(l.id, p.id, g.id)

                    for s in g.members:
                        matching.s_matched.append(s)
...
matching.print_group()

jsonData = {
    'project_matched': [json.loads(json.dumps(matching.p_matched))],
    'group_matched': [json.loads(json.dumps(matching.g_matched))],
    'lecturer_matched': [json.loads(json.dumps(matching.l_matched))],
    'status': 'success'
}
